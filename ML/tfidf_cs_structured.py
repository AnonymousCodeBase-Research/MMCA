...
# Compute average cosine similarity for all attribute pairs
def compute_aggregated_similarity(df, vectorizer, attributes):
    """Compute aggregated average cosine similarity for all attribute pairs."""
    aggregated_scores = []
    ...
    for index in range(len(df)):
        ...
        for left_attr, right_attr in attributes:
            ...
            # Compute cosine similarity and add to total
            total_similarity += cosine_similarity(m1, m2)[0][0]
        # Compute average similarity
        avg_similarity = total_similarity / num_attributes
        aggregated_scores.append(avg_similarity)
...
for dataset_name in dataset_name_list:
    for dataset_index in range(10):
        ...
        # Compute aggregated cosine similarity for validation and test datasets
        valid_scores = compute_aggregated_similarity(valid, vectorizer, attributes)
        test_scores = compute_aggregated_similarity(test, vectorizer, attributes)
        ...
        with open('parameter_tuning_tfidf_cs.txt', 'a') as f:
