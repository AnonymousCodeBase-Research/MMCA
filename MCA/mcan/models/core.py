...
from .modules import RNN, GatingMechanism, SelfAttention, PairAttention, GlobalAttention, \
    Fusion
...
class MCANModel(nn.Module):
    ...
    def initialize(self, train_dataset, init_batch=None):
        ...
        self.gru = RNN('gru', hidden_size=self.hidden_size, dropout=self.rnn_dropout)
        ...
        self.gate_mechanism = GatingMechanism(hidden_size=self.hidden_size,
                                              dropout=self.gate_dropout)
        self.global_attention = GlobalAttention(hidden_size=self.hidden_size,
                                                style="dot",
    ...
    def forward(self, input):
        ...
        return self.classifier(entity_comparison), left_summary, right_summary
...
class BinaryClassifier(nn.Sequential):
    def __init__(self):
        ...
        self.add_module('softmax_transform',
                        mcan.modules.Transform('1-layer',
                                               non_linearity=None,
                                               output_size=2))
