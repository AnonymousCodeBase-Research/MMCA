...
from mcan_structured.models.core import MCANModel
from mcan_structured.optim import Optimizer, SoftNLLLoss
from mcan_structured.runner import Runner as AAMRunner
...
PAPER_DROPOUT = 0.1
WEIGHT_DECAY  = 1e-6
IGNORE_COLUMNS = ['query', 'source']  # safe default

# === grid ===
GRID: List[Dict[str, Any]] = [
    dict(batch=batch, lr=lr, lbl_smooth=0.05, attn_heads=1, dropout=PAPER_DROPOUT)
...
# -----------------------------
# AAM helpers (silent score pass)
# -----------------------------
def aam_get_scores(model, dataset_obj, batch_size, path, dataset_index) -> pd.DataFrame:
    preds = AAMRunner._run(
        ...
        for dataset_index in range(10):
            ...
            for params in GRID:
                ...
                for epoch in range(PAPER_EPOCHS):
                    ...
                    t0_tr = time.time()
                    train_scores = aam_get_scores(model, train, batch_size=params['batch'],
                    ...
                    # VALID sweep
                    t0 = time.time()
                    valid_scores = aam_get_scores(model, valid, batch_size=params['batch'],
                                                  path=dataset, dataset_index=dataset_index)
            ...
            test_scores = aam_get_scores(refit_model, test_refit, batch_size=best_params['batch'],
                                         path=dataset, dataset_index=dataset_index)
