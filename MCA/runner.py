...
class Runner(object):
    ...
    def predict(model, dataset, output_attributes=False, **kwargs):
        ...
        # Default to silent inference, unless caller overrides.
    ...
    @staticmethod
    def _compute_scores(path, dataset_index, output, target, test, cosine):
    ...
    def _run(path,
        ...
        if verbose:
            ...
            total_ticks = 0
            tick_every = 10**9  # effectively never
            tick_count = 0
            pbar = None
        ...
        for batch_idx, batch in enumerate(run_iter):
            ...
            if train:
                output, left_representation, right_representation = model(batch)
                ...
                if hasattr(batch, label_attr):
                    scores = Runner._compute_scores(
                        path, dataset_index, output, getattr(batch, label_attr), 0,
                        nn.functional.cosine_similarity(left_representation, right_representation)
                    )
                else:
                ...
                with torch.no_grad():
                    ...
                    if hasattr(batch, label_attr):
                        scores = Runner._compute_scores(
                            path, dataset_index, output, getattr(batch, label_attr),
                            1 if run_type == 'TEST' else 0,
                            nn.functional.cosine_similarity(left_representation, right_representation)
                        )
                    else:
                        scores = (0.0, 0.0, 0.0, 0.0)
        ...
        # per-epoch TRAIN callback with epoch-level metrics (to mirror DeepMatcher)
        ...
        Runner._print_final_stats(epoch + 1, runtime, datatime, cum_stats, test=(run_type == 'TEST'), verbose=verbose)

        if return_predictions:
            return predictions
        else:
            # return F1 (percent scale) to keep parity with DeepMatcher’s train loop
            return cum_stats.f1()
    ...
    def train(model,
              ...
              alpha=0.0,
              **kwargs):
        """Train an MCA model; returns best validation F1 (percent)."""
        # accept epoch_callback + verbose from kwargs
        ...
        for epoch in epochs_range:
            ...
            # TRAIN — emits epoch_callback with epoch-level metrics
            Runner._run(
            ...
            score = Runner._run(path, dataset_index, 'EVAL', model, validation_dataset,
