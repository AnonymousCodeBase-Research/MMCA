    ...
    def _forward(self, input_with_meta, meta_data=None):
        ...
        if self.use_meta:
            if input_with_meta.lengths is not None:
                ...
                attn_score.data.masked_fill_(binary_mask == 0, -float('inf'))
    ...
    def _forward(self, input_with_meta, raw_input_with_meta=None):
        ...
        for head in range(self.heads):
            ...
            if input_with_meta.lengths is not None:
                ...
                binary_mask = (mask > 0).float()
                alignment_scores.data.masked_fill_(binary_mask == 0, -float('inf'))
                # alignment_scores.data.masked_fill_(~mask, 0)
    ...
    def _forward(self, input_with_meta):
        ...
        if self.style == 'last':
            ...
            if input_with_meta.lengths is not None:
                ...
                binary_mask = (mask > 0).float().cuda()
                input.data.masked_fill_(binary_mask == 0, -float('inf'))
